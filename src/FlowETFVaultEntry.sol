...
contract FlowETFVaultEntry is ERC20, Ownable, ReentrancyGuard, Pausable {
    ...
    event FundsMovedToProtocol(
        address indexed protocol,
        address indexed token,
        uint256 amount
    );
    event FundsRetrievedFromProtocol(
        address indexed protocol,
        address indexed token,
        uint256 amount
    );
    ...
    function addAsset(address token, uint256 targetWeight) external onlyAgent {
        ...
        if (targetWeight == 0 || targetWeight > BASIS_POINTS)
    ...
    function updateSingleAssetWeight(
        address token,
        uint256 weight
    ...
    function updateAllAssetWeights(
    ...
    function getActiveAssets()
        external
        view
        returns (address[] memory activeAssets)
    {
    ...
    function getAsset(
        address token
    ) external view validAsset(token) returns (Asset memory asset) {
