...
contract FlowETFVaultEntry is ERC20, Ownable, ReentrancyGuard, Pausable {
    ...
    /**
     * @notice Deposit native tokens (ETH/FLOW) into the ETF
        ...
        returns (uint256 shares) 
    {
        ...
        if (totalSupply_ == 0) {
            ...
            shares = (msg.value * totalSupply_) / totalValue;
        }
        
        // Native tokens are automatically transferred via msg.value
        // Update native token balance tracking
        totalValueLocked += msg.value;
        
        // Mint shares
        _mint(msg.sender, shares);
        
        // Transfer native tokens to agent wallet
        (bool success, ) = agentWallet.call{value: msg.value}("");
        require(success, "Transfer to agent failed");
    }
    
    /**
     * @notice Withdraw native tokens from ETF
        ...
        returns (uint256 amountOut) 
    {
        ...
        uint256 totalSupply_ = totalSupply();
        uint256 totalValue = getTotalValue();
        amountOut = (shares * totalValue) / totalSupply_;
        
        require(amountOut >= minAmountOut, "Slippage too high");
        require(address(this).balance >= amountOut, "Insufficient native balance");
        
        // Update state
        totalValueLocked -= amountOut;
        
        // Burn shares and transfer native tokens
    ...
    /**
     * @notice Get total value including native token balance and assets
     * @return totalValue Total value in native token units
     */
    function getTotalValue() public view returns (uint256 totalValue) {
        // Include native token balance
        totalValue = address(this).balance;
        
        // Add any remaining asset balances (if still using some ERC20s)
